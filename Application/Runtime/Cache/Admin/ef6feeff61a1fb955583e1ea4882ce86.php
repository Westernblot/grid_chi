...
<body onload="init()">
     ...
     <object classid="clsid:88638732-1B2B-4BD8-AA32-8159092E9F51" id=LaCtrl CODEBASE="*.cab#version=0,0,0,0"  width=500 height=340> 
     </object>
...
<tr>
	...
	<br>
	 ...
	 <input   TYPE="button"   VALUE="拍照并上传显示"   onClick="shotToServer()"> 
...
<script type="text/javascript">
	...
	//拍照上传到服务器
	function shotToServer(){
			//alert(LaCtrl.CaptureAsBase64(idx));
			var base64=LaCtrl.CaptureAsBase64(idx);
			//alert(base64);
			$.ajax({
				url:'/grid_chi/index.php/Admin/Gpy/decodeBase64ToImg',
				type:'post',
				dataType: 'json',
                data: {'base64':base64},
                ...
                var name=data.name+".jpg";
                var url="/grid_chi/Uploads/gpy/"+name;
                var href = url;
               ...
               // alert(name+";"+url);
				...
				var html = '<div>'
						    +'<input type="hidden" name="gpy[]" value="'+name+'">'
	...
	function fuzhi(){
        ...
        parent.$('#div_gpy_attachment').text('我被改变了');
