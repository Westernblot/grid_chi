				...
				<div class="row-fluid">
					...
					<div class="well you-bg">
						<form id="form1" action="/grid_chi/index.php/Admin/Project/cmdProjectAudit?toPage=<?php echo ($toPage); ?>&flag=<?php echo ($flag); ?>&add_dept=<?php echo ($add_dept); ?>&s_date=<?php echo ($s_date); ?>&e_date=<?php echo ($e_date); ?>" method="post">
							...
							<input type="hidden" name="field_flag" value="tc_flag" />          <!-- 审核字段 -->
							<input type="hidden" name="field_opinion" value="tc_opinion" />    <!-- 审核意见字段 -->
     ...
     (
         ...
         function removeParent(obj){
         	$(obj).parent().remove();
							...
							<font color="red"><h3>开工信息<select onchange="toggle(this,'div_kg')">
								  <option value="隐藏">隐藏</option>
								  <option value="展开">展开</option>
								  </select></h3></font>
							<hr>
							<!-- 签约项目模板 -->
							<div id="div_kg" style="display: none">
							<table class="youAdd tabAudit">
	<tr>
     ...
     (
         ...
         function removeParent(obj){
         	$(obj).parent().remove();
                 ...
                 <?php if(($mo["tc_flag"]) != "审核通过"): ?><h3>本次审核意见：</h3>
