			...
			<div class="container-fluid">
				<div class="row-fluid">
					...
					<div class="btn-toolbar">
						<a class="btn btn-danger pull-right" onclick="resetAudit('zc_flag','审核中')">
							撤销审核
						</a> 
						<form class="form-inline" id="form1" action="/grid_chi/index.php/Admin/Project/auditZcProjectMain" method="get">
						...
						时间：
						<input type="text" id="s_date" name="s_date" value="<?php echo ($s_date); ?>" class="Wdate"  onclick="WdatePicker({dateFmt:'yyyyMM'})" />
						至
						<input type="text" id="e_date" name="e_date" value="<?php echo ($e_date); ?>" class="Wdate"  onclick="WdatePicker({dateFmt:'yyyyMM'})" />
						...
						<?php if(empty($list)): ?><tr>
							<td colspan="20" align="center"><font color="red">查无数据</font></td>
								...
								<?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr id="<?php echo ($vo["id"]); ?>" <?php if(($k%2) == "0"): ?>class="odd"
										...
										<td><?php echo ($vo["add_dept"]); ?></td>
										<td><?php echo (subtext($vo["qy_outside_investor"],12)); ?></td>
										<td>
											<a href="/grid_chi/index.php/Admin/Project/auditZcProjectUI?id=<?php echo ($vo["id"]); ?>&flag=<?php echo ($flag); ?>&add_dept=<?php echo ($add_dept); ?>&s_date=<?php echo ($s_date); ?>&e_date=<?php echo ($e_date); ?>"><?php if(($vo["qy_total_invest"]) >= "5"): ?><img src="/grid_chi/Public/images/icon/flag.png"/><?php endif; ?>
										...
										<td>
											<a href="/grid_chi/index.php/Admin/Project/seeProjectUI?id=<?php echo ($vo["id"]); ?>&showNum=3">查看</a></td>
										<td>
											<a href="/grid_chi/index.php/Admin/Project/zcProjectUI?id=<?php echo ($vo["id"]); ?>">编辑</a> 
											<?php if(($vo["zc_flag"]) == "审核中"): ?><a href="/grid_chi/index.php/Admin/Project/auditZcProjectUI?id=<?php echo ($vo["id"]); ?>">审核</a><?php endif; ?>
										</td> -->
